...
void notify_pc(){
	uint8_t notify_pc[5]={0xff};
	AttsHandleValueNtf(1, WP_DAT_HDL, 1, notify_pc);
}

void ask_for_data_len(){
	uint8_t data_len_notifaction[5]={0xff};
	AttsHandleValueNtf(1, WP_DAT_HDL, 2, data_len_notifaction);
}

bool_t get_connected_status( ){
	return periphCb.connected;
}


void send_data_packet(dmConnId_t connId, uint16_t handle, uint16_t index ){
	uint8_t * my_data =get_rx_data();
	int start  = (int)index*500;
	int end  = (int)index*500+500;


	uint8_t new_data[501];
	int k=0;
	new_data[k]=(uint8_t) index;
	k++;

	for( int i=start; i<end; i++){

		new_data[k] = my_data[i];


		k++;
	}

	AttsHandleValueNtf(connId, WP_DAT_HDL, 501, new_data);
                           ...
                           uint16_t len, uint8_t *pValue, attsAttr_t *pAttr)
{
	...
	uint16_t index = (uint16_t)*pValue;
	if(pValue[0]==0xFF && pValue[1]==0xFF ){
		set_data_len((int)pValue[2]);
		return ATT_SUCCESS;
	...
	if(index==0){
			printf("test start of transaction\n");
			set_send_flag(1);
	}

	if (AttsCccEnabled(connId, PERIPH_WP_DAT_CCC_IDX)) {
		send_data_packet(connId,handle,index);
	}
	if(index==get_data_len()){
